...
import noogel.xyz.search.infrastructure.queue.AbstractJobProcessor;
...
@Service
@Slf4j
public class IndexContentJobProcessor extends AbstractJobProcessor {
    private final ExecutorService EXECUTORS = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
    ...
    private boolean addToVectorStore(IndexedContentDto dto) {
        try {
            ...
            for (int i = 0; i < documents.size(); i += 5) {
                int end = Math.min(i + 5, documents.size());
                List<Document> subList = documents.subList(i, end);
                vectorClient.getVectorStore().add(subList);
                log.info("添加向量数据 success {} percent {}/{}", dto.getResId(), end, documents.size());
            }
            return true;
        } catch (Exception ex) {
