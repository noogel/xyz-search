...
public class ElasticClient {
    ...
    public void updateSettings() {
        ConfigProperties.Elastic sc = configProperties.getApp().getChat().getElastic();
        if (Objects.isNull(holder)) {
            return;
        }
        ElasticsearchClient client = holder.getClient();
        // 更新索引设置
        Optional.ofNullable(sc.getHighlightMaxAnalyzedOffset())
                .filter(l -> l > 1000_000)
                .ifPresent(k -> {
                    try {
                        client.indices().putSettings(l -> l
                                .settings(s -> s.highlight(h -> h.maxAnalyzedOffset(k))));
                        log.info("更新索引设置 highlightMaxAnalyzedOffset={} success.", k);
                    } catch (IOException e) {
                        log.error("更新索引设置 highlightMaxAnalyzedOffset={} failed.", k, e);
                    }
                });
    }
}